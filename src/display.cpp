...
Display::Display(System* memory) : m_system(memory), m_last_cycle(0), m_frameReady(false) {}
...
void Display::ExecuteHDMATransferBlock(uint32 bytes)
{
  ...
  // check address range
  TRACE("HDMA transfer 0x%04X -> 0x%04X 0x%03X (%u) bytes", source_address, destination_address, copy_length,
        copy_length);
