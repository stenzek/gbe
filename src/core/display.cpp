...
Display::Display(System* memory) : m_system(memory) {}
...
uint8 Display::CPUReadRegister(uint8 index) const
{
  ...
  WARNING_LOG("Unhandled LCD register read: {:02X}", index);
  return 0x00;
}
...
void Display::CPUWriteRegister(uint8 index, uint8 value)
{
  ...
  WARNING_LOG("Unhandled LCD register write: {:02X} (value {:02X})", index, value);
}
...
void Display::Reset()
{
  ...
  std::memset(m_cgb_bg_palette, 0, sizeof(m_cgb_bg_palette));
  std::memset(m_cgb_sprite_palette, 0, sizeof(m_cgb_sprite_palette));
...
bool Display::DoState(StateWrapper& sw)
{
  sw.Do(&m_last_cycle);

  // Write registers
  sw.Do(&m_registers.LCDC);
  sw.Do(&m_registers.STAT);
  sw.Do(&m_registers.SCY);
  sw.Do(&m_registers.SCX);
  sw.Do(&m_registers.LY);
  sw.Do(&m_registers.LYC);
  sw.Do(&m_registers.BGP);
  sw.Do(&m_registers.OBP0);
  sw.Do(&m_registers.OBP1);
  sw.Do(&m_registers.WY);
  sw.Do(&m_registers.WX);
  sw.Do(&m_registers.HDMA1);
  sw.Do(&m_registers.HDMA2);
  sw.Do(&m_registers.HDMA3);
  sw.Do(&m_registers.HDMA4);
  sw.Do(&m_registers.HDMA5);
  sw.Do(&m_registers.BGPI);
  sw.Do(&m_registers.OBPI);

  // Write cgb palettes
  sw.DoBytes(m_cgb_bg_palette, sizeof(m_cgb_bg_palette));
  sw.DoBytes(m_cgb_sprite_palette, sizeof(m_cgb_sprite_palette));

  // Write state
  sw.Do(&m_state);
  sw.Do(&m_modeClocksRemaining);
  sw.Do(&m_HDMATransferClocksRemaining);
  sw.Do(&m_cyclesSinceVBlank);
  sw.Do(&m_currentScanLine);
...
void Display::SetLCDCRegister(uint8 value)
{
  ...
  if (lcd_current_state != lcd_new_state)
  {
    ...
    if (!lcd_new_state)
    {
      ...
      TRACE_LOG("Display disabled.");
      ClearFrameBuffer();
      PushFrame();
    }
    else
    {
      TRACE_LOG("Display enabled.");
    }
  }
...
void Display::SetHDMA5Register(uint8 value)
{
  ...
  if (mode == 0)
  {
    ...
    if (old_value & 0x80)
    {
      ...
      TRACE_LOG("Cancelling HBLANK HDMA transfer (remaining: 0x{:03X}/{} bytes)", remaining, remaining);
      if (old_remaining != remaining)
        WARNING_LOG("HDMA Cancel - amounts differ (0x{:03X} vs 0x{:03X})", old_remaining, remaining);
...
void Display::ExecuteHDMATransferBlock(uint32 bytes)
{
  ...
  // Writing to FF55 starts the transfer, the lower 7 bits of FF55 specify the Transfer Length (divided by 10h, minus
  // 1). Ie. lengths of 10h-800h bytes can be defined by the values 00h-7Fh.
  uint32 length = uint32((m_registers.HDMA5 & 0x7F) + 1) * 0x10;
  uint32 copy_length = std::min(length, bytes);

  // The Source Start Address may be located at 0000-7FF0 or A000-DFF0, the lower four bits of the address are ignored
  // (treated as zero). The Destination Start Address may be located at 8000-9FF0, the lower four bits of the address
  // are ignored (treated as zero), the upper 3 bits are ignored either (destination is always in VRAM).
  uint16 current_source_address = (source_address & 0xFFF0);
  uint16 current_destination_address = (destination_address & 0x1FF0);

  // check address range
  TRACE_LOG("HDMA transfer 0x{:04X} -> 0x{:04X} 0x{:03X} ({}) bytes", source_address, destination_address, copy_length,
        copy_length);
  if ((source_address > 0x7FF0 && source_address < 0xA000) || source_address > 0xDFF0)
    WARNING_LOG("Source address out of range (0x{:04X})", source_address);
...
void Display::RenderScanline(uint8 LINE)
{
  ...
  std::memset(pFrameBufferLine, 0xFF, SCREEN_WIDTH * 4);
  if (!IsDisplayEnabled())
    return;
  ...
  uint32 background_palette[4] = {
    grayscale_colors[m_registers.BGP & 0x3], grayscale_colors[(m_registers.BGP >> 2) & 0x3],
    grayscale_colors[(m_registers.BGP >> 4) & 0x3], grayscale_colors[(m_registers.BGP >> 6) & 0x3]};

  // read sprite palettes
  uint32 obj_palette0[4] = {0xFF555555, grayscale_colors[(m_registers.OBP0 >> 2) & 0x3],
                            grayscale_colors[(m_registers.OBP0 >> 4) & 0x3],
                            grayscale_colors[(m_registers.OBP0 >> 6) & 0x3]};
  uint32 obj_palette1[4] = {0xFF555555, grayscale_colors[(m_registers.OBP1 >> 2) & 0x3],
                            grayscale_colors[(m_registers.OBP1 >> 4) & 0x3],
                            grayscale_colors[(m_registers.OBP1 >> 6) & 0x3]};
  ...
  if (SPRITE_ENABLE)
  {
    ...
    {
      std::qsort(active_sprites, num_active_sprites, sizeof(active_sprites[0]), [](const void* a, const void* b) -> int {
      ...
      num_active_sprites = std::min(num_active_sprites, (uint32)10);
    }
  }
  ...
  for (uint32 pixel_x = 0; pixel_x < 160; pixel_x++)
  {
    ...
    if (SPRITE_ENABLE)
    {
      ...
      for (uint32 sprite_index = 0; sprite_index < num_active_sprites; sprite_index++)
      {
        ...
        // "In 8x16 mode, the lower bit of the tile number is ignored. Ie. the upper 8x8 tile is "NN AND FEh", and the
        // lower 8x8 tile is "NN OR 01h"."
        uint16 tile_index;
        if (SPRITE_SIZE_BIT)
        {
          if (tile_y >= 8)
...
{
  R2Video.UploadFramebuffer(SCREEN_WIDTH, SCREEN_HEIGHT, RETRO2_PIXEL_FORMAT_RGBA8, m_frameBuffer, SCREEN_WIDTH * 4);
...
void Display::DumpTiles(uint8 tilemap)
{
  ...
  DEV_LOG("BGMAPBASE = {:04X}", BGMAPBASE);
  ...
  if (BG_ENABLE)
  {
    ...
    for (uint32 gy = 0; gy < 32; gy++)
    {
      ...
      auto inserter = std::back_inserter(msg);
      fmt::format_to(inserter, "Group {}: ", gy);
