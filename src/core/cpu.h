...
public:
  ...
  uint32 GetCycles() const { return m_clock; }
  ...
  static bool Disassemble(char* buf, size_t buf_len, System* memory, uint16 address);
...
private:
  ...
  inline uint8 MemReadByte(uint16 address)
  {
    DelayCycle();
    return m_system->CPURead(address);
  }
  inline void MemWriteByte(uint16 address, uint8 value)
  {
    DelayCycle();
    m_system->CPUWrite(address, value);
  }
  inline uint16 MemReadWord(uint16 address)
  {
    return ((uint16)MemReadByte(address)) | ((uint16)MemReadByte(address + 1) << 8);
  }
  inline void MemWriteWord(uint16 address, uint16 value)
  {
    MemWriteByte(address, (uint8)(value));
    MemWriteByte(address + 1, (uint8)(value >> 8));
  }
